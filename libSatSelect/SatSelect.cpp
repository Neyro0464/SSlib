...
//        if (VelocityCheck(this->station, Satellites.back())) {
//            Satellites.pop_back();
//        }
//    }
//    file.close();
...
bool SetelliteSelect::SetStationPos() {
    Station station;
    ...
    this->station.geo = station.geo;
    this->station.dec = station.dec;
    return true;
}

//Enter station vision parametres
bool SetelliteSelect::SetFilter()
{
    Station station;
...
void SetelliteSelect::SetTLEFile()
{
    std::string file = "";
...
std::vector<NORAD_DATA> SetelliteSelect::GetSatArray() {

    std::vector<NORAD_DATA> Objects;
    COORDS dummy;
    auto stationtmp = this->station;
    int N = this->Satellites.size();      // number of satellites that passed verification by speed
